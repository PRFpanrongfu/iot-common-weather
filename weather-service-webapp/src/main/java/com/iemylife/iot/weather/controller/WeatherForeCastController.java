...
public class WeatherForeCastController extends BaseController {
    ...
    @Autowired
    private WeatherDataDailyInfoServicesImpl weatherDataDailyInfoService;
    ...
    public ResponseEntity<?> searchByCode(@RequestParam String code) {
        try {
            ...
            //将获取的数据留存到数据库
            //weatherDataDailyInfoService.insertBatch(weatherDataDailyInfo);
            List<WeatherDataDailyInfo> weatherDataDailyInfo = new ArrayList<>();
            //从数据库获取数据
            List<WeatherDataDailyInfo> weatherDataDailyInfo1 = weatherDataDailyInfoService.selectByCode(code);
            if (weatherDataDailyInfo1 != null) {
                weatherDataDailyInfo = weatherDataDailyInfo1;
            }
            //从api获取数据
            List<WeatherDataDailyInfo> weatherDataDailyInfo2 = new ArrayList<>();
            weatherDataDailyInfo2 = weatherDataDailyInfoService.getWeatherForecastFromAPIByCode(code);
            if (weatherDataDailyInfo2 != null) {
    ...
    public ResponseEntity<?> searchByCity(@RequestParam String city) {
        try {
            List<WeatherDataDailyInfo> weatherDataDailyInfo = new ArrayList<>();
            List<WeatherDataDailyInfo> weatherDataNowInfo1 = weatherDataDailyInfoService.selectByCity(city);
            if (weatherDataNowInfo1 != null) {
                weatherDataDailyInfo = weatherDataNowInfo1;
            }
            List<WeatherDataDailyInfo> weatherDataDailyInfo2 = weatherDataDailyInfoService.getWeatherForecastFromAPIByCity(city);
            if (weatherDataDailyInfo2 != null) {
                weatherDataDailyInfo = weatherDataDailyInfo2;
            }
            List<Object> modelResponseList = new ArrayList<>();
